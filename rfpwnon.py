...
lbl = Label(window, text="Frequency: " + str(freq * 0.000001) + "Mhz", fg ="green", bg="black").pack()
...
#transmit
lbl = Label(window, text="Transmitting", fg ="green", bg="black").pack()
count =0
def counter():
    global count
while(startn < brutelength):
    #Label(window, text=count).pack()
    count = counter()
    for i in range(0,results.repeatTimes):
        try:
            ...
            print ("")
            print ("Transmitting...")
            #lbl = Label(window, text="Select Frequency").pack()
            #lbl1 = Label(window, text="Made By Abdullah Alawdan").pack()
            ...
            if(results.keys):
                ...
                #lbl = Label(window, text="Binary before pwm encoding").pack()
                print (brutepackettemp)
                #lbl = Label(window, text=(brutepackettemp)).pack()
                print ("Binary after pwm encoding:")
                #lbl = Label(window, text="Binary after pwm encoding").pack()
                print (pwmbin)
                listbox.insert(2, brutepackettemp)
                listbox.pack()
                #lbl = Label(window, text=(pwmbin)).pack()
                count = brutepackettemp
                window.update_idletasks()
                window.update()

        except Exception as e:
            messagebox.showerror("Error", "Lost communication to USB device.. waiting 3 seconds, then retrying.")
...
print ("")
#messagebox.showinfo("Title", "a Tk MessageBox" + str(freq) + "s")
messagebox.showinfo('Done',
                                'All ' + str(binL) + '-bit codes has been successfully sent on frequency ' + str(freq * 0.000001) + 'Mhz with baud rate of ' + str(baudRate) + '.\n'
                                'Click OK to scroll through all the sent codes\n'
                                '© GUI Made By Abdullah Alawdan\n')
print ("Done.")
window.mainloop()
d.setModeIDLE()
